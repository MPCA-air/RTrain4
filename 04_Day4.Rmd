...
![](images/day4_sq.png){width="260" style="float: left; margin-right: 60px; margin-top: 18px;"}
...
- Your `dpylr` toolbox
- Conditional mutate
    - Use `ifelse()` for _Yes/No_ decisions
- More plots
    - New geoms
    - Colors
    - Faceting/Splitting by groups
    - Titles 
- Break out Finn
- __Concatulations!__
...
###  Bad news <i class="fas fa-coins"></i> {-}
...
`ifelse( `__Lightsaber__ <span style="color: green;">__GREEN__ is?</span>, __Yes!__ ` Then it's Yoda's,` __No!__ ` Then Yoda's it is not)`

</div>

<br>

Or say we wanted to label all the porgs over _60 cm_ as tall, and everyone else as short.  Whenever we want to add a column where the value depends on the value found in another column. We can use `ifelse()`.
...
>
> <i class="far fa-question-circle" aria-hidden="true"></i> Who remembers how to plot???
>
...
### <i class="fa fa-cogs" aria-hidden="true" style="color:#040707;"></i> EXERCISE {-}
...
```{r, eval=F}
cheap_scrap <- filter(scrap,   ______   ==   "_________"  )
...
__Step 2:__ Make a `geom_col()` plot showing the total pounds of Black boxes shipped to each destination. 
...
>
> __Woop! Honestly, most people don't survive this long.__ 
>
> Super-serious kudos to you. You have earned yourself a great award. 
>
> Stay tuned for something to arrive by snail-<i class="fas fa-envelope"></i>.
...
<details><summary> _[Click here for a HINT]_ </summary>
<p>

_This hint intentionally left blank. Sorry, you're on your own._
...
Venture forth and make a boxplot. We've covered how to do a scatterplot with `geom_point` and a column chart with `geom_col`, but how would you make a boxplot showing the prices at each destination? You're on your own here. Feel free to seek help and to spice things up with `color` ,`facet_wrap`, `theme`, and `labs`. 
...
# Save it!
ggsave(filename = "Save my plot here.png", plot = my_plot)
```
...
When you add more layers using `+` remember to place it at the __end__ of each line.
